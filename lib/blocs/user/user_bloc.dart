...
class UserBloc extends Bloc<UserEvent, UserState> {
  ...
  UserBloc(this.userRepository)
      : super(UserState(errorMessage: "", succesMessage: "", status: FormsSatus.pure , profileModel: ProfileModel.initial())) {
  ...
  _deleteUserCollection(DeleteUserCollectionEvent event , emit) async {
    NetworkResponse  networkResponse  = await userRepository.deleteUser(event.profileModel.userId);
    if(networkResponse.errorText.isEmpty){
      emit(state.copyWith(status: FormsSatus.auth));
    }
    else{
  ...
  _updateUserCollection(UpdateUserCollectionEvent event , emit) async {
    NetworkResponse  networkResponse  = await userRepository.updateUser(event.profileModel);
    if(networkResponse.errorText.isEmpty){
      emit(state.copyWith(status: FormsSatus.auth));
    }
    else{
  ...
  _getUserByDocId(GetUserByDocId event , emit) async {
    NetworkResponse  networkResponse  = await userRepository.getUserByDocId(event.docId);
    if(networkResponse.errorText.isEmpty){
      emit(state.copyWith(status: FormsSatus.auth));
    }
    else{
